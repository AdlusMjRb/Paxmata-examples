...
/**
 * @title HighRiskPool
 * @dev ERC20-based lending pool for high-risk projects (SCORS 5.0-6.99) with non-transferable tokens
 */
contract HighRiskPool is ERC20, ReentrancyGuard {
    // ============================================================
    ...
    // Risk parameters - High Risk specific
    uint256 public maxExposurePerProject = 25 ether;  // Max 25 ETH per project
    uint256 public maxUtilizationRate = 7500;         // Max 75% of funds can be deployed
    uint256 public dailyOutflowLimit = 75 ether;      // Max 75 ETH outflow per day
    uint256 public maxWalletPercentage = 1500;        // Max 15% of pool per wallet (basis points)
    uint256 public minScorsRequirement = 500;         // Minimum SCORS of 5.0 (basis points)
    uint256 public maxScorsRequirement = 699;         // Maximum SCORS of 6.99 (basis points)
    uint256 public baseInterestRate = 1300;           // 13% base interest rate (basis points)
    uint256 public maxInterestRate = 1800;            // 18% maximum interest rate (basis points)
    ...
    /**
     * @notice Initializes the high risk lending pool with non-transferable ERC20 tokens
     * @param _registry Address of the central registry
     */
    constructor(address _registry) 
        ERC20("Paxmata High Risk Pool Token", "PAXT-H") 
    {
    ...
    function _transfer(
        ...
        require(
            ...
            "PAXT-H tokens are non-transferable"
        );
    ...
    function approve(address /* spender */, uint256 /* amount */) public pure override returns (bool) {
        revert("PAXT-H tokens cannot be approved for spending");
    }
    ...
    function transferFrom(address /* sender */, address /* recipient */, uint256 /* amount */) public pure override returns (bool) {
        revert("PAXT-H tokens are non-transferable");
    }
    ...
    function userApplyForLoan(
        ...
        require(_userScors >= minScorsRequirement, "SCORS too low for High Risk pool");
        require(_userScors <= maxScorsRequirement, "SCORS too high for High Risk pool");
...
function _disburseLoanInternal(uint256 _projectId, address _escrowAddress) internal {
    ...
    /**
     * @notice Calculate interest rate based on SCORS (High Risk specific)
    ...
    function calculateInterestRate(uint256 _scors) internal view returns (uint256) {
        // At SCORS 6.99 (699): minimum rate of 13% (1300 basis points)
        // At SCORS 5.0 (500): maximum rate of 18% (1800 basis points)
