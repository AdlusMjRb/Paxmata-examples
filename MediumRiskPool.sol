...
/**
 * @title MediumRiskPool
 * @dev ERC20-based lending pool for medium-risk projects (SCORS 7.0-8.99) with non-transferable tokens
 */
contract MediumRiskPool is ERC20, ReentrancyGuard {
    // ============================================================
    ...
    // Risk parameters - Medium Risk specific
    uint256 public maxExposurePerProject = 50 ether;  // Max 50 ETH per project
    uint256 public maxUtilizationRate = 8000;         // Max 80% of funds can be deployed
    uint256 public dailyOutflowLimit = 150 ether;     // Max 150 ETH outflow per day
    uint256 public maxWalletPercentage = 1500;        // Max 15% of pool per wallet (basis points)
    uint256 public minScorsRequirement = 700;         // Minimum SCORS of 7.0 (basis points)
    uint256 public maxScorsRequirement = 899;         // Maximum SCORS of 8.99 (basis points)
    uint256 public baseInterestRate = 800;            // 8% base interest rate (basis points)
    uint256 public maxInterestRate = 1200;            // 12% maximum interest rate (basis points)
    ...
    /**
     * @notice Initializes the medium risk lending pool with non-transferable ERC20 tokens
     * @param _registry Address of the central registry
     */
    constructor(address _registry) 
        ERC20("Paxmata Medium Risk Pool Token", "PAXT-M") 
    {
    ...
    function _transfer(
        ...
        require(
            ...
            "PAXT-M tokens are non-transferable"
        );
    ...
    function approve(address /* spender */, uint256 /* amount */) public pure override returns (bool) {
        revert("PAXT-M tokens cannot be approved for spending");
    }
    ...
    function transferFrom(address /* sender */, address /* recipient */, uint256 /* amount */) public pure override returns (bool) {
        revert("PAXT-M tokens are non-transferable");
    }
    ...
    function userApplyForLoan(
        ...
        require(_userScors >= minScorsRequirement, "SCORS too low for Medium Risk pool");
        require(_userScors <= maxScorsRequirement, "SCORS too high for Medium Risk pool");
...
function _disburseLoanInternal(uint256 _projectId, address _escrowAddress) internal {
    ...
    /**
     * @notice Calculate interest rate based on SCORS (Medium Risk specific)
    ...
    function calculateInterestRate(uint256 _scors) internal view returns (uint256) {
        // At SCORS 8.99 (899): minimum rate of 8% (800 basis points)
        // At SCORS 7.0 (700): maximum rate of 12% (1200 basis points)
        if (_scors >= maxScorsRequirement) return baseInterestRate; // Minimum rate for best score in tier
        
        // Linear interpolation between min and max rates
        uint256 scoreRange = maxScorsRequirement - minScorsRequirement;
        uint256 rateRange = maxInterestRate - baseInterestRate;
        uint256 scoreFromTop = maxScorsRequirement - _scors;
